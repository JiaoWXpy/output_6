...
def count_list_shu(data_list):
    ...
    output = []
    count_0 = 0
    list3 = return_list3_shu(5, data_list)
    list3_all = []
    # print(list_4)
    for i, data in enumerate(data_list):
        if i < 5:
            ...
            if not str_in_list3_shu(data, list3):
                output.append(0)
                count_0 += 1
            else:
                output.append(count_0 + 1)
                count_0 = 0
                list3_tmp = return_list3_shu(i + 1, data_list)
                if list3_tmp != False:
                    list3 = list3_tmp
            # print(output)
            list3_all.append("".join(list3))
...
def return_list3_shu(i, data_list):
    list5 = data_list[i - 5:i]
    # print(list5)
    list5.reverse()
    list3 = []
...
def str_in_list3_shu(str_data, list_3):
    in_list = 0
    if str_data[-1] in list_3:
        return True
    else:
...
def read_rule():
    ...
    for i in rule_list_str[1:]:
        ...
        rule_list_1 = [float(x) for x in i[3:]]
        ...
        for i, data in enumerate(current_data_all):
            ...
            # 从第二个数字开始
                ...
                # 斜向
                    ...
                    in_list3_shu = False
                #竖向
                elif rule_map[currnt_rule] > 0 and rule_map[currnt_rule] < 1:
                    print("竖")
                    if rule_count == 0 and not in_list3_shu:
                        list3_tmp = return_list3_shu(i, current_data_all)
                        if list3_tmp != False:
                            list3 = list3_tmp
                    if not str_in_list3_shu(data, list3):
                        result["result"].append(0)
                        ...
                        list3_tmp = return_list3_shu(i + 1, current_data_all)
                        if list3_tmp != False:
                            list3 = list3_tmp
                        rule_count += 1

                        if rule_count >= np.abs(10 * rule_map[currnt_rule]):
                            rule_count = 0
                            currnt_rule += 1
                        if currnt_rule >= len(rule_map):
                            currnt_rule = 0
